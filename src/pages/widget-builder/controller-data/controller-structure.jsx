...
const ControllerStructure = (props) => {
    ...
    const innerController = () => {
        ...
        if ('text' == controller_data?.type) {
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel',])} />
                    <input className='cwk-controller-input' type="text" placeholder={controller_data?.placeHolder ?? ''} value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <input className='cwk-controller-input' type="number" placeholder={controller_data?.placeHolder ?? ''} value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
                </div>
            )
        } else if ('textarea' == controller_data?.type) {
            return (
                <div className='cwk-controller-large' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <textarea className='cwk-controller-input' placeholder={controller_data?.placeHolder ?? ''} value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} rows={controller_data?.rows ?? 1} />
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <div className='cwk-controller-input-container'>
            ...
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <input className='cwk-controller-input-color' type="color" value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
                </div>
            )
        } else if ('select' == controller_data?.type) {
            const select_content = () => {
                return (
                    <div className='cwk-controller-select-content'>
                        {controller_data?.options?.map((option, index) => (
                            <span className='cwk-controller-select-content-item' key={index} onClick={() => { updateValue('defaultValue', option.value) }}>{option.label}</span>
                        ))}
                    </div>
                )
            }
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <div className='cwk-controller-dropdown'>
                        <Dropdown
            ...
            return (
                <div className='cwk-controller-large'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <div className='crafpa-controller-unit-content'>
                        <div className='cwk-controller-dropdown-item'>
                            <input className='cwk-controller-dropdown' type="number" value={(getValue('top'))} onChange={(e) => { updateController('dimension_defaultValue', 'top', e.target.value) }} />
                            <span className='cwk-controller-dropdown-item-label'>Top</span>
                        </div>
                        <div className='cwk-controller-dropdown-item'>
                            <input className='cwk-controller-dropdown' type="number" value={(getValue('right'))} onChange={(e) => { updateController('dimension_defaultValue', isLinked ? 'top' : 'right', e.target.value) }} />
                            <span className='cwk-controller-dropdown-item-label'>Right</span>
                        </div>
                        <div className='cwk-controller-dropdown-item'>
                            <input className='cwk-controller-dropdown' type="number" value={(getValue('bottom'))} onChange={(e) => { updateController('dimension_defaultValue', isLinked ? 'top' : 'bottom', e.target.value) }} />
                            <span className='cwk-controller-dropdown-item-label'>Bottom</span>
                        </div>
                        <div className='cwk-controller-dropdown-item'>
                            <input className='cwk-controller-dropdown' type="number" value={(getValue('left'))} onChange={(e) => { updateController('dimension_defaultValue', isLinked ? 'top' : 'left', e.target.value) }} />
                            <span className='cwk-controller-dropdown-item-label'>Left</span>
                        </div>
                        <span className={`cwk-controller-linked ${controller_data?.dimension_defaultValue?.isLinked ? 'cwk-controller-linked-active' : ''}`} onClick={(e) => { e.stopPropagation(); updateController('dimension_defaultValue', 'isLinked', !isLinked) }}>
            ...
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <span className='cwk-controller-value-icon'>
                        <i className='cwk-i-typo-controller'></i>
                    </span>
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <span className='cwk-controller-value-icon'>
                        <i className='cwk-i-textshadow-controller'></i>
                    </span>
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <span className='cwk-controller-value-icon'>
                        <i className='cwk-i-boxshadow-controller'></i>
                    </span>
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <span className='cwk-controller-value-icon'>
                        <i className='cwk-i-border-controller'></i>
                    </span>
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container'>
                    ...
                    {Array.isArray(controller_data?.types) &&
                        <div className='cwk-controller-cwk-i-wrapper'>
                            ...
                            {controller_data?.types.includes('gradient') &&
                                <span className='cwk-controller-value-icon'>
                                    <i className='cwk-i-background-controller'></i>
                                </span>
                            }
                        </div>
                    }
            ...
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                    <span className='cwk-controller-value-icon'>
                        <i className='cwk-i-color-wheel-controller'></i>
                    </span>
                </div>
            )
        } else if ('divider' == controller_data?.type) {
            return (
                <hr className='cwk-controller-separator' />
            )
        } else if ('normalhover' == controller_data?.type) {
            return (
                <div className='cwk-controller-large'>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel',])} />
                    <div className='cwk-normalhover-controller'>
                        <div className='cwk-normalhover-tabs'>
                            {Array.isArray(controller_data?.nha_array) &&
                                controller_data?.nha_array.map((item, index) => (
                                    <span
                                        className={`cwk-normalhover-tab ${item.value == controller_data?.nha_type ? 'cwk-normalhover-tab-active' : ''}`}
                                        key={index}
                        ...
                        <div className='cwk-normalhover-content'>
            ...
            return (
                <div className='cwk-controller-large' style={getStyle(['labelBlock'])}>
                    ...
                    <div className='crafpa-controller-renge-content'>
                        <input className='cwk-controller-range-input'
                            type="range"
                            min={getValue('min')}
                            max={getValue('max')}
                            step={getValue('step')}
                        ...
                        <div className='crafpa-controller-unit-content'>
                            <div className='cwk-controller-dropdown-item'>
                                <input className='cwk-controller-dropdown'
                                    type="number"
                                    min={getValue('min')}
                                    max={getValue('max')}
                                    step={getValue('step')}
            ...
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel',])} />
                    <input className='cwk-controller-input' type="text" value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
                </div>
            )
            ...
            return (
                <div className='cwk-controller-large' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel'])} />
                    <textarea className='cwk-controller-input' placeholder={controller_data?.placeHolder ?? ''} value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} rows={2} />
                </div>
            )
        } else if ('preview' == controller_data?.type) {
            return (
                <div className='cwk-controller-container'>
                    <input className='cwk-controller-label' type="text" value={'Update Preview Changes'} readOnly />
                    <span className='cwk-controller-preview-button'>{__('Update', 'captain-widgets-kit')}</span>
                </div>
            )
            ...
            return (
                <div className='cwk-controller-container' style={getStyle(['labelBlock'])}>
                    <input className='cwk-controller-label' type="text" value={controller_data?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} style={getStyle(['showlabel',])} />
                    <div className='cwk-controller-url-content'>
                        <input className='cwk-controller-input' type="text" placeholder={controller_data?.placeHolder ?? ''} value={controller_data?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
                        <span className='cwk-controller-setting-icon'><i className='cwk-i-setting'></i></span>
                    </div>
                </div>
            )
        }
    ...
    return (
        <Fragment>
            {'before' == controller_data?.separator && <hr className='cwk-controller-separator' />}
            <div className={`cwk-controller-wrapper ${checkActiveController() ? 'cwk-controller-wrapper-active' : ''}`} onClick={(e) => { e.stopPropagation(); handleActiveController() }}>
                {innerController()}
                {controller_data?.description && <span className='cwk-controller-description'>{controller_data?.description}</span>}
                <div className='cwk-controller-actions'>
                    {!hideEditControlls.includes(controller_data?.type) &&
                        <span className='cwk-controller-action-btn'>
                            <i className='cwk-i-edit'></i>
                        </span>
                    }
                    <span className='cwk-controller-action-btn' onClick={(e) => { e.stopPropagation(); deleteController() }}>
                        <i className='cwk-i-delete'></i>
                    </span>
                </div>
            </div>
            {'after' == controller_data?.separator && <hr className='cwk-controller-separator' />}
        </Fragment>
    )
}
