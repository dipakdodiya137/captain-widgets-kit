...
const Fields = () => {
    ...
    const TooltipSvg = (<i className='captwiki-i-information-1'></i>);
    ...
    const getDefaultInputType = () => {
        ...
        const select_content = () => {
            return (
                <div className='captwiki-dp-container'>
                    {selected_controller?.options?.map((option, index) => (
                        <span className='captwiki-dp-item' key={index} onClick={() => { updateValue('defaultValue', option.value) }}>{option.value}</span>
                    ))}
                </div>
            )
        }
        ...
        if ('number' == selected_controller?.type) {
            ...
            return (
                <div className='captwiki-fields-input-container'>
            ...
            return (
                <div className='captwiki-fields-input-container captwiki-fields-input-container-slider'>
                    <input className='captwiki-fields-input' type="number"
        ...
        return (
            <input className='captwiki-fields-input' type={inputType} placeholder={__('Default Value', 'captain-widgets-kit')} value={selected_controller?.defaultValue ?? ''} onChange={(e) => { updateValue('defaultValue', e.target.value) }} />
        )
    }
    ...
    const separator_content = () => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateValue('separator', 'default') }}>{__('Default', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('separator', 'before') }}>{__('Before', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('separator', 'after') }}>{__('After', 'captain-widgets-kit')}</span>
            </div>
        )
    };
    ...
    const selector_type_content = () => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateValue('selector_type', 'basic') }}>{__('Basic', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('selector_type', 'advanced') }}>{__('Advanced', 'captain-widgets-kit')}</span>
            </div>
        )
    };
        ...
        return (
            <div className='captwiki-dp-container'>
                {selected_controller?.size_units?.map((unitData, index) => {
                    if (unitData.checked) {
                        return (
                            <span className='captwiki-dp-item' key={index} onClick={() => { updateController('defaultValue', 'unit', unitData.type) }}>{unitData.type}</span>
                        )
                    }
                })}
            </div>
    ...
    const condition_relation_content = () => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateController('condition_value', 'relation', 'and') }}>{__('And', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('condition_value', 'relation', 'or') }}>{__('Or', 'captain-widgets-kit')}</span>
            </div>
        )
    };
    ...
    const condition_operator_content = (index) => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateConditionValue(selected_controller?.condition_value?.values, index, 'operator', '==') }}>==</span>
                <span className='captwiki-dp-item' onClick={() => { updateConditionValue(selected_controller?.condition_value?.values, index, 'operator', '!=') }}>!=</span>
            </div>
        )
    };
    ...
    const language_content = () => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateValue('language', 'html') }}>{__('HTML', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('language', 'css') }}>{__('CSS', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('language', 'javascript') }}>{__('JavaScript', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateValue('language', 'php') }}>{__('PHP', 'captain-widgets-kit')}</span>
            </div>
        )
    };
        ...
        return (
            <div className='captwiki-dp-container'>
                {input_type_array.map((item, index) => (
                    <span className='captwiki-dp-item' key={index} onClick={() => { updateValue('input_type', item.value) }}>{item.label}</span>
                ))}
            </div>
        )
    };
    ...
    const addAlignOption = () => {
        ...
        const newOption = {
            ...
            align_icon: 'captwiki-i-text-align-left',
            align_title: '',
            align_svg: ''
        }
    ...
    const align_option_content = (index) => {
        return (
            <div className='captwiki-dp-container'>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-text-align-left', index) }}>{__('Align Left', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-text-align-right', index) }}>{__('Align Right', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-text-align-center', index) }}>{__('Align Center', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-text-align-justify', index) }}>{__('Align Justify', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-arrow-up', index) }}>{__('Up Arrow', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-arrow-down', index) }}>{__('Down Arrow', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-arrow-right', index) }}>{__('Right Arrow', 'captain-widgets-kit')}</span>
                <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', 'eicon-i-arrow-left', index) }}>{__('Left Arrow', 'captain-widgets-kit')}</span>
                {/* <span className='captwiki-dp-item' onClick={() => { updateController('align_option', 'align_icon', '', index) }}>Custom Icon</span> */}
            </div>
        )
    }
    ...
    const accordion_header = (label) => {
        return (
            <div className='captwiki-fields-accordion-header'>
                <span className='captwiki-fields-accordion-header-label'>{label}</span>
            </div>
        )
    }
    ...
    const advanced_selectors_content = () => {
        return (
            <div className='captwiki-fields-input-container captwiki-fields-selectors-advanced'>
                <div className='captwiki-fields-selectors-advanced-php'>{`'selectors' => array(`}</div>
                <CodeEditor lang={'php'} value={selected_controller?.selectors_advanced ?? ''} changeEvent={(value) => { updateValue('selectors_advanced', value) }} />
                <div className='captwiki-fields-selectors-advanced-php'>{`)`}</div>
            </div>
        )
    }
    ...
    return (
        <div className='captwiki-controller-fields-wrapper'>
                        ...
                        <>
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    <span className='captwiki-fields-label-container'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Name Value', 'captain-widgets-kit')} value={selected_controller?.name ?? ''} readOnly />
                                        <span className='captwiki-controller-copy-icon' onClick={() => { copyVariableName(selected_controller?.name) }}>
                            ...
                            }
                            {
                                ...
                                <div className={`captwiki-fields-content ${selected_controller?.type == 'code' ? 'captwiki-fields-content-column' : ''}`}>
                                    <span className='captwiki-fields-label-container'>
                        ...
                        <>
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Label Value', 'captain-widgets-kit')} value={selected_controller?.label ?? ''} onChange={(e) => { updateValue('label', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                        ...
                        <>
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container captwiki-fields-label-container-dropdown'>
                                        <Dropdown
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    <span className='captwiki-fields-label-container'>
                                    ...
                                    <div className='captwiki-fields-unit-wrapper'>
                                        <div className='captwiki-fields-unit-contaner'>
                                            <input className='captwiki-fields-input' type="number" placeholder={__('Min', 'captain-widgets-kit')} value={selected_controller?.number_setting?.min ?? ''} onChange={(e) => { updateController('number_setting', 'min', e.target.value) }} />
                                            <span className='captwiki-fields-unit-item'>{__('Min', 'captain-widgets-kit')}</span>
                                        </div>
                                        <div className='captwiki-fields-unit-contaner'>
                                            <input className='captwiki-fields-input' type="number" placeholder={__('Max', 'captain-widgets-kit')} value={selected_controller?.number_setting?.max ?? ''} onChange={(e) => { updateController('number_setting', 'max', e.target.value) }} />
                                            <span className='captwiki-fields-unit-item'>{__('Max', 'captain-widgets-kit')}</span>
                                        </div>
                                        <div className='captwiki-fields-unit-contaner'>
                                            <input className='captwiki-fields-input' type="number" placeholder={__('Step', 'captain-widgets-kit')} value={selected_controller?.number_setting?.step ?? ''} onChange={(e) => { updateController('number_setting', 'step', e.target.value) }} />
                                            <span className='captwiki-fields-unit-item'>{__('Step', 'captain-widgets-kit')}</span>
                                        </div>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-unit-wrapper'>
                                        <div className='captwiki-fields-unit-contaner'>
                                            <input className='captwiki-fields-input' type="text" placeholder={__('Switch Is Off', 'captain-widgets-kit')} value={selected_controller?.label_on ?? ''} onChange={(e) => { updateValue('label_on', e.target.value) }} />
                                            <span className='captwiki-fields-unit-item'>{__('On', 'captain-widgets-kit')}</span>
                                        </div>
                                        <div className='captwiki-fields-unit-contaner'>
                                            <input className='captwiki-fields-input' type="text" placeholder={__('Switch Is On', 'captain-widgets-kit')} value={selected_controller?.label_off ?? ''} onChange={(e) => { updateValue('label_off', e.target.value) }} />
                                            <span className='captwiki-fields-unit-item'>{__('Off', 'captain-widgets-kit')}</span>
                                        </div>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Custom Attributes', 'captain-widgets-kit')} value={selected_controller?.custom_attributes ?? ''} onChange={(e) => { updateValue('custom_attributes', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-options-container'>
                                        <div className='captwiki-fields-options-header'>
                                            <span className='captwiki-fields-options-header-item'>{__('Unit', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Min', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Max', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Step', 'captain-widgets-kit')}</span>
                                        </div>
                                        {selected_controller?.size_units?.map((unitData, index) => {
                                            return (
                                                <div className='captwiki-fields-options-item'>
                                                    <label className='captwiki-size-unit-label' htmlFor={`captwiki-size-unit-${unitData?.type}`}>
                                                        <Checkbox id={`captwiki-size-unit-${unitData?.type}`} checked={unitData.checked} onChange={(e) => { updateController('size_units', 'checked', e.target.checked, index) }} />
                                                        <span>{unitData?.type}</span>
                                                    </label>
                                                    <input className='captwiki-fields-input' type="number" placeholder={__('Min', 'captain-widgets-kit')} value={unitData.min} onChange={(e) => { updateController('size_units', 'min', e.target.value, index) }} />
                                                    <input className='captwiki-fields-input' type="number" placeholder={__('Max', 'captain-widgets-kit')} value={unitData.max} onChange={(e) => { updateController('size_units', 'max', e.target.value, index) }} />
                                                    <input className='captwiki-fields-input' type="number" placeholder={__('Step', 'captain-widgets-kit')} value={unitData.step} onChange={(e) => { updateController('size_units', 'step', e.target.value, index) }} />
                                                </div>
                                            )
                                        })}
                                    </div>
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-options-container'>
                                        <div className='captwiki-fields-options-header'>
                                            <span className='captwiki-fields-options-header-item'>{__('Label', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Value', 'captain-widgets-kit')}</span>
                                        </div>
                                        {selected_controller?.options?.map((option, index) => {
                                            return (
                                                <div className='captwiki-fields-options-item'>
                                                    <input className='captwiki-fields-input' type="text" placeholder='Label' value={option.label} onChange={(e) => { updateController('options', 'label', e.target.value, index) }} />
                                                    <input className='captwiki-fields-input' type="text" placeholder='Label' value={option.value} onChange={(e) => { updateController('options', 'value', e.target.value, index) }} />
                                                    <span className={`captwiki-fields-options-item-delete ${selected_controller?.options?.length == 1 ? 'captwiki-fields-options-item-delete-disabled' : ''}`}
                                    ...
                                    <span className='captwiki-fields-options-add' onClick={() => { addOption() }}>{__('Add Option', 'captain-widgets-kit')}</span>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Return Value', 'captain-widgets-kit')} value={selected_controller?.return_value ?? ''} onChange={(e) => { updateValue('return_value', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-options-container'>
                                        <div className='captwiki-fields-options-header'>
                                            <span className='captwiki-fields-options-header-item'>{__('Label', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Value', 'captain-widgets-kit')}</span>
                                            <span className='captwiki-fields-options-header-item'>{__('Icon', 'captain-widgets-kit')}</span>
                                        </div>
                                        {selected_controller?.align_option?.map((option, index) => {
                                            return (
                                                <div className='captwiki-fields-options-item'>
                                                    <input className='captwiki-fields-input' type="text" placeholder={__('Label', 'captain-widgets-kit')} value={option.align_label} onChange={(e) => { updateController('align_option', 'align_label', e.target.value, index) }} />
                                                    <input className='captwiki-fields-input' type="text" placeholder={__('Label', 'captain-widgets-kit')} value={option.align_value} onChange={(e) => { updateController('align_option', 'align_value', e.target.value, index) }} />
                                                    <Dropdown
                                                        title={(<input className='captwiki-options-header-item' value={option.align_icon} readOnly />)}
                                                        content={align_option_content(index)}
                                                    />
                                                    <span className={`captwiki-fields-options-item-delete ${selected_controller?.align_option?.length == 1 ? 'captwiki-fields-options-item-delete-disabled' : ''}`}
                                    ...
                                    <span className='captwiki-fields-options-add' onClick={() => { addAlignOption() }}>{__('Add Align Option', 'captain-widgets-kit')}</span>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <textarea className='captwiki-fields-input' placeholder='Description' value={selected_controller?.description ?? ''} onChange={(e) => { updateValue('description', e.target.value) }} rows='3'></textarea>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Placeholder Value', 'captain-widgets-kit')} value={selected_controller?.placeHolder ?? ''} onChange={(e) => { updateValue('placeHolder', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="number" placeholder={__('Number of Rows', 'captain-widgets-kit')} value={selected_controller?.rows ?? ''} min={1} onChange={(e) => { updateValue('rows', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-unit-wrapper'>
                                        <label className='captwiki-normalhover-label' htmlFor='captwiki-normalhover-normal'>
                                            <Checkbox id='captwiki-normalhover-normal' checked={selected_controller?.nha_array?.find(item => item.value == 'normal') ? true : false} onChange={(e) => { updateNhaArray('normal', 'Normal', e.target.checked) }} />
                                            {__('Normal', 'captain-widgets-kit')}
                                        </label>
                                        <label className='captwiki-normalhover-label' htmlFor='captwiki-normalhover-hover'>
                                            <Checkbox id='captwiki-normalhover-hover' checked={selected_controller?.nha_array?.find(item => item.value == 'hover') ? true : false} onChange={(e) => { updateNhaArray('hover', 'Hover', e.target.checked) }} />
                                            {__('Hover', 'captain-widgets-kit')}
                                        </label>
                                        <label className='captwiki-normalhover-label' htmlFor='captwiki-normalhover-active'>
                                            <Checkbox id='captwiki-normalhover-active' checked={selected_controller?.nha_array?.find(item => item.value == 'active') ? true : false} onChange={(e) => { updateNhaArray('active', 'Active', e.target.checked) }} />
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-unit-wrapper'>
                                        <label className='captwiki-fields-type-wrapper' htmlFor='captwiki-bg-classic'>
                                            <Checkbox checked={selected_controller?.types?.includes('classic') ?? false} onChange={(e) => { updateArray('types', 'classic', e.target.checked) }} id='captwiki-bg-classic' />
                                            {__('Classic', 'captain-widgets-kit')}
                                        </label>
                                        <label className='captwiki-fields-type-wrapper' htmlFor='captwiki-bg-gradient'>
                                            <Checkbox checked={selected_controller?.types?.includes('gradient') ?? false} onChange={(e) => { updateArray('types', 'gradient', e.target.checked) }} id='captwiki-bg-gradient' />
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('Classes', 'captain-widgets-kit')} value={selected_controller?.controlClass ?? ''} onChange={(e) => { updateValue('controlClass', e.target.value) }} />
                                    </div>
                                </div>
                            }
                        </>
                        ...
                        <>
                            ...
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <textarea className='captwiki-fields-input' placeholder={__('CSS Selectors', 'captain-widgets-kit')} value={selected_controller?.selectors ?? ''} onChange={(e) => { updateValue('selectors', e.target.value) }} rows='3'></textarea>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <span className='captwiki-fields-options-add' onClick={() => { handlePopup('selectors_advanced') }}>{__('Add Selectors', 'captain-widgets-kit')}</span>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <textarea className='captwiki-fields-input' placeholder={__('CSS Selector', 'captain-widgets-kit')} value={selected_controller?.selector ?? ''} onChange={(e) => { updateValue('selector', e.target.value) }} rows='3'></textarea>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <input className='captwiki-fields-input' type="text" placeholder={__('CSS Property', 'captain-widgets-kit')} value={selected_controller?.selector_value ?? ''} onChange={(e) => { updateValue('selector_value', e.target.value) }} />
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <textarea className='captwiki-fields-input' placeholder={__('CSS Property', 'captain-widgets-kit')} value={selected_controller?.selector_value ?? ''} onChange={(e) => { updateValue('selector_value', e.target.value) }} rows='3'></textarea>
                                    </div>
                                </div>
                            }
                            {
                                ...
                                <div className='captwiki-fields-content captwiki-fields-content-column'>
                                    ...
                                    <div className='captwiki-fields-input-container'>
                                        <textarea className='captwiki-fields-input' placeholder={__('CSS Property value', 'captain-widgets-kit')} value={selected_controller?.property_value ?? ''} onChange={(e) => { updateValue('property_value', e.target.value) }} rows='3'></textarea>
                                    </div>
                                </div>
                            }
                        </>
                        ...
                        <>
                            {
                                ...
                                <div className='captwiki-multiple-content'>
                                    <div className='captwiki-fields-content'>
                                        <span className='captwiki-fields-label-container'>
                                    ...
                                    {selected_controller?.conditions &&
                                        <>
                                            ...
                                            <div className='captwiki-fields-content captwiki-fields-content-column'>
                                                <div className='captwiki-fields-options-container'>
                                                    <div className='captwiki-fields-options-header'>
                                                        <span className='captwiki-fields-options-header-item'>{__('Name', 'captain-widgets-kit')}</span>
                                                        <span className='captwiki-fields-options-header-item'>{__('Operator', 'captain-widgets-kit')}</span>
                                                        <span className='captwiki-fields-options-header-item'>{__('Value', 'captain-widgets-kit')}</span>
                                                    </div>
                                                    {selected_controller?.condition_value?.values?.map((condition, index) => {
                                                        return (
                                                            <div className='captwiki-fields-options-item'>
                                                                <input className='captwiki-fields-input' type="text" placeholder='Label' value={condition.name} onChange={(e) => { updateConditionValue(selected_controller?.condition_value?.values, index, 'name', e.target.value) }} />
                                                                <Dropdown
                                                                    title={condition.operator ?? ''}
                                                                    content={condition_operator_content(index)}
                                                                />
                                                                <input className='captwiki-fields-input' type="text" placeholder='Label' value={condition.value} onChange={(e) => { updateConditionValue(selected_controller?.condition_value?.values, index, 'value', e.target.value) }} />
                                                                <span className={`captwiki-fields-options-item-delete ${selected_controller?.condition_value?.values?.length == 1 ? 'captwiki-fields-options-item-delete-disabled' : ''}`}
                                                ...
                                                <span className='captwiki-fields-options-add' onClick={() => { addConditionValue() }}>{__('Add Condition Value', 'captain-widgets-kit')}</span>
                                            </div>
                                        </>
                                    }
                                </div>
