 ...
 * @since 0.0.1
 */
if ( ! class_exists( 'Captwiki_Animated_Border_2_L3pomo25' ) ) {
	...
	class Captwiki_Animated_Border_2_L3pomo25 {
		...
		public function captwiki_enqueue_style() {
			$custom_css_url = CAPTWIKI_URL . 'includes/patch-load/patchs/animated-border-2-l3pomo25/animated-border-2-l3pomo25.css';
			wp_enqueue_style( 'captwiki-wpl6jf26-animated-border-2-l3pomo25', $custom_css_url, array(), time() );
		}
		...
		public function captwiki_register_controls( $element ) {
			...
			$element->start_controls_section(
				'section-uxxg9z25',
				array(
					'label'   => esc_html__( 'Animated Border 2', 'captain-widgets-kit' ),
			...
			$element->add_control(
				'switcher_1ibu1s25',
				array(
					'label'        => esc_html__( 'Enable Animated Border 2', 'captain-widgets-kit' ),
		...
		public function captwiki_render_content( $content, $element ) {
			...
			$dd_style = isset( $settings['switcher_1ibu1s25'] ) ? $settings['switcher_1ibu1s25'] : '';

			if ( 'Yes' === $dd_style ) {
				$element->add_render_attribute( '_wrapper', 'class', 'captwiki-animated-border-two' );
	...
	Captwiki_Animated_Border_2_L3pomo25::get_instance();
}
